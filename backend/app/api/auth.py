...
async def login(body: LoginRequest, db: AsyncSession = Depends(get_db)):
    ...
    if redis_client:
        await redis_client.setex(f"refresh:{refresh_token}", 7 * 86400, str(user.id))
...
async def refresh(body: RefreshRequest, db: AsyncSession = Depends(get_db)):
    ...
    if redis_client:
        stored = await redis_client.get(f"refresh:{body.refresh_token}")
        if not stored:
    ...
    if redis_client:
        await redis_client.setex(f"refresh:{new_refresh}", 7 * 86400, str(user.id))
